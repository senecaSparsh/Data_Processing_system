...
namespace sdds_ws9 {
	...
	int ProcessData::operator()(std::string fileName, double& avg, double& var) {
		...
		std::vector<std::thread> procedure1;
		for (int i = 0; i < num_threads; i++) {
			procedure1.push_back(std::thread(computeAvg, &data[p_indices[i]], std::ref(averages[i])));
		}
		for (auto& ele : procedure1) {
			ele.join();
		}
		for (int i = 0; i < num_threads; i++) {
			avg += averages[i];
		...
		std::vector<std::thread> procedure2;
		for (int i = 0; i < num_threads; i++) {
			procedure2.push_back(std::thread(computeVar, &data[p_indices[i]], std::ref(variances[i])));
		}
		for (auto& ele : procedure2) { 
			ele.join();
		}
		for (int i = 0; i < num_threads; i++) {
			var += variances[i];
